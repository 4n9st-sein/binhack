...
int main(int argc, char **argv){
	...
	//allocate memory 
	void *load_addr = mmap(NULL, MEM_SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED | MAP_NORESERVE, -1, 0);
	...
	fread(load_addr, 1, MEM_SIZE, file);
	fclose(file);
	...
	void *kvm_run_ptr = mmap(NULL, kvm_run_mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED, vcpu_fd, 0);
	if (kvm_run_ptr < 0){
		perror("mmap failed");
		return 1;
	}
	...
	//first bit of rflag is always 1, reset other registers 
	regs.rflags = 1 << 1;
	regs.rip  = 0;
	regs.rsp  = 0xffffffff;
	regs.rbp  = 0;
	...
	while (1) {
		...
		// if KVM_RUN returns, handle KVM_EXIT_REASON accordinglly
