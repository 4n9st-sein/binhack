...
int main(void) {
  ...
  char *head = (char *)(mmap(NULL, mapped_size, PROT_READ |PROT_WRITE, MAP_PRIVATE, fd, 0));
  ...
  for (size_t pi = 0; pi < ehdr->e_phnum; pi++) {
    ...
    if (phdr->p_type == PT_NOTE) {
        ...
        while(offset_in_note < phdr->p_filesz) {
          Elf64_Nhdr *note = (Elf64_Nhdr *)(head + phdr->p_offset + offset_in_note);
          uint32_t type = note->n_type;
          if (type == NT_PRSTATUS) {
            prstatus_t *prstatus = (prstatus_t *)(head + phdr->p_offset + offset_in_note + 4 * 3 + (note->n_namesz +3)/4 * 4);
            struct user_regs_struct *regs = (struct user_regs_struct *)(prstatus->pr_reg);
            printf("RIP: 0x%llx\n", regs->rip);
          }
          offset_in_note += sizeof(Elf64_Nhdr) + (note->n_descsz + 3)/4 *4 + (note->n_namesz + 3) /4 *4;
        }
      }
    }
  }
